...
def _ensure_email_verification_schema():
    try:
        ...
        print(f"Warning: failed to inspect email verification schema: {exc}")
        return
    ...
    try:
        with engine.begin() as conn:
            conn.execute(
                text("ALTER TABLE users ADD COLUMN is_verified BOOLEAN NOT NULL DEFAULT TRUE")
            )
    except Exception as exc:
        print(f"Warning: failed to add is_verified column: {exc}")


_ensure_email_verification_schema()
...
def issue_email_verification_code(db: Session, email: str) -> str:
    normalized_email = normalize_email(email)
    code = generate_verification_otp()
    expires_at = datetime.now() + timedelta(minutes=EMAIL_OTP_EXPIRY_MINUTES)
